...
def create_budget(
    ...
    if isinstance(allocation, StandardBudget):
        ...
        standard_budgets = [b for b in existing_budgets if isinstance(b, StandardBudget)]
        
        # 1. Child Check: Cannot exceed Parent
        parts = allocation.account.split(":")
        if len(parts) > 1:
            parent_name = ":".join(parts[:-1])
            parent_budget = next((b for b in standard_budgets if b.account == parent_name), None)
            
            if parent_budget:
                # Siblings (Direct children of parent)
                siblings = [
                    b for b in standard_budgets 
