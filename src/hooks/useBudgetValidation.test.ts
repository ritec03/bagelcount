...
function createTestWrapper(
  ...
  const extendedBudgets = facade.initializeBudgets(budgets, constraintConfig);
  useAppStore.getState().updateBudgetList(extendedBudgets);
  return ({ children }: { children: React.ReactNode }) =>
    React.createElement(
      ConstraintConfigContext.Provider,
...
function renderValidationHook(
  ...
  ...args: Parameters<typeof useBudgetValidation>
) {
  const wrapper = createTestWrapper(budgets ?? [], CONSTRAINT_CONFIG);
  return renderHook(() => useBudgetValidation(...args), { wrapper });
}
...
describe("useBudgetValidation", () => {
  ...
  it("should detect insufficient parent for children (WARNING only)", () => {
    ...
    expect(result.current.warnings[0]).toContain("Children sum (1000) exceeds parent budget (800) by 200.");
  });
...
describe("useBudgetValidation (TDD Spec)", () => {
  ...
  describe("Child Validation (Warning Rules)", () => {
    it("should WARN if new Parent budget is too small for existing Children", () => {
      ...
      const budgets = [
        createBudget("Expenses:Food:Restaurants", "500", "monthly"),
      ];
    ...
    it("should verify Same-Account Hierarchy (Monthly exists, masks Children)", () => {
      ...
      const validHook = renderValidationHook(
        budgets,
        "Expenses:Food",
        6000,
        "StandardBudget",
      ...
      const invalidHook = renderValidationHook(
        budgets,
        "Expenses:Food",
        5000,
        "StandardBudget",
...
describe("Red Team: useBudgetValidation Edge Cases", () => {
  ...
  it("should NOT warn when lower-frequency budget exceeds higher-frequency equivalent (normal case)", () => {
    ...
    const budgets = [createBudget("Expenses:Food", "500", "monthly")];
  ...
  it("should allow a lower-frequency budget that exceeds a higher-frequency equivalent (no warning)", () => {
    ...
    const budgets = [createBudget("Expenses:Food", "200", "monthly")];
  ...
  it("should BLOCK a higher-frequency budget that exceeds the lower-frequency equivalent", () => {
    ...
    const budgets = [createBudget("Expenses:Food", "3000", "yearly")];
